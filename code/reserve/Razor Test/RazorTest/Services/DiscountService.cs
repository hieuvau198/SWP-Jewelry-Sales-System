...
namespace RazorTest.Services
{
    public class DiscountService
    {
        ...
        {
            return await _apiService.GetAsync<List<Discount>>("http://localhost:5156/api/discount");
        }

        public async Task<Discount> GetDiscountByIdAsync(string discountId)
        {
            return await _apiService.GetAsync<Discount>($"http://localhost:5156/api/discount/{discountId}");
        }

        public async Task<HttpResponseMessage> CreateDiscountAsync(Discount discount)
        {
            return await _apiService.PostAsJsonAsync("http://localhost:5156/api/discount", discount);
        }
        ...
        {
            return await _apiService.DeleteAsync($"http://localhost:5156/api/discount/{discountId}");
        }
