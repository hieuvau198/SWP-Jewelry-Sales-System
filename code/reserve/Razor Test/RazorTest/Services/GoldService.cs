...
namespace RazorTest.Services
{
    public class GoldService
    {
        ...
        {
            return await _apiService.GetAsync<List<Gold>>("http://localhost:5156/api/gold");
        }

        public async Task<Gold> GetGoldByIdAsync(string goldId)
        {
            return await _apiService.GetAsync<Gold>($"http://localhost:5156/api/gold/{goldId}");
        }

        public async Task<HttpResponseMessage> CreateGoldAsync(Gold gold)
        {
            return await _apiService.PostAsJsonAsync("http://localhost:5156/api/gold", gold);
        }

        public async Task<HttpResponseMessage> UpdateGoldAsync(Gold gold)
        {
            var url = "http://localhost:5156/api/gold";
        ...
        {
            return await _apiService.DeleteAsync($"http://localhost:5156/api/gold?goldId={goldId}");
        }
